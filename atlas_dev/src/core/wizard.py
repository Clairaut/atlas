...
from atlas_dev.src.models.celestial_state import CelestialState, PHASE_DEFS, ELONGATION_EVENTS
...
class Wizard:
	...
	def conjure_celestial_state(
		...
		if "position" in properties:
			for frame in frames:
				if frame in ("ecliptic", "equatorial"):
					self._observatory.project(frame)
				else:
					self._observatory.orient(frame)
				pos = self._observatory.observe(target_info["id"])
	...
	def conjure_celestial_trace(
		...
		while current <= end_dt:
			state = self.conjure_celestial_state(
				dt         = current,
				location   = location,
				target     = target,
				zodiac     = zodiac,
	...
	def _scan_ingresses(
		...
		from atlas_dev.src.models.celestial_state import SIGNS
		events: list[Event] = []
