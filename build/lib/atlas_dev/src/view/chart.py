...
from atlas_dev.src.models.aspect import Aspect, ASPECT_DEFS, ASPECT_GLYPHS
...
class PlaybackChart(LiveRadixChart):
    ...
    def key_event(self, key: Any, action: Any, modifiers: Any):
    ...
    # Earth's sidereal rotation rate (deg/s) — used to interpolate house cusps between frames
    SIDEREAL_RATE = 360.0 / 86164.1

    # Rebuild geometry with positions extrapolated by t ∈ [0, 1] using planetary velocity
    def _rebuild_interpolated(self, t: float):
        from copy import copy
        cls       = self.__class__
        step_secs = cls._play_step.total_seconds()
        step_days = step_secs / 86400.0
        ...
        # Interpolate house cusps using Earth's sidereal rotation rate
        cusp_advance  = self.SIDEREAL_RATE * step_secs * t
        interp_cusps  = [(c + cusp_advance) % 360.0 for c in cls._cusps]
